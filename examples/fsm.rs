...
app! {
    ...
    idle: {
        resources: [LED, END, TX, CB, BUTTON],
    },
    tasks: {
        USART1: {
            priority: 1,
...
fn idle(mut t: &mut Threshold, mut r: idle::Resources) -> ! {
    ...
    finite_machine.state = StateType::Auto;
    ...
    loop {
        ...
        if r.END.claim(&mut t, |end, _| *end) {
            ...
            //         _ => StateType::SteadyState,
            // };

            // finite_machine.state = state;

            //if state == auto tentar quebrar o frame.
            match finite_machine.state {
                StateType::Auto => {
                    let op = vec.get(4);
